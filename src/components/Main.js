...
const generateCategories = props.categories.map(category => {
    return (
        <option id={category.id} value={category.id} key={nanoid()}>{category.name}</option>
    )
})
    ...
    return (
        <main className="home">
            ...
            <select onChange={props.difficulty} name="difficulty" value={props.quizDifficulty} className="selectDifficulty" id="difficulty">
